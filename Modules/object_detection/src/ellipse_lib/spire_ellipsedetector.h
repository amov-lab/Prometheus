...
/**
suffixs: For example, ".jpg|.png" (split by '|')
*/
...
/***************** rand-related functions ****************/
/** select m from n, store in arr
*/
...
public:
    ...
    bool IsValid() {
        ...
        return xc_ > 0 && yc_ > 0 && a_ > 0 && b_ > 0 && rad_ > 0;
    }
};
...
public:
    ...
    void Detect_ThreePoint(cv::Mat3b& I, std::vector<Ellipse>& ellipses);
...
private:
    ...
    void QAConstraint(std::vector<VVP>& segments8, std::vector<VVP>& segments_return, std::vector<EllipseThreePoint>& ellipse_tps);
    void EllipseFitThreePoint(std::vector<EllipseThreePoint>& ellipse_tps, std::vector<Ellipse>& ellipses);
