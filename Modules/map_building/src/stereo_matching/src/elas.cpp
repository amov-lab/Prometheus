    ...
    for (int32_t u_can = 0; u_can < D_can_width; u_can++) {
        for (int32_t v_can = 0; v_can < D_can_height; v_can++) {
            ...
            if (d_can >= 0) {
                ...
                int32_t support = 0;
                for (int32_t u_can_2 = u_can - param.incon_window_size;
                     u_can_2 <= u_can + param.incon_window_size; u_can_2++) {
                    for (int32_t v_can_2 = v_can - param.incon_window_size;
                         v_can_2 <= v_can + param.incon_window_size; v_can_2++) {
        ...
        for (int32_t i = 0; i < num_grid; i++) {
            ...
            if (d_curr < d_plane_min || d_curr > d_plane_max) {
                ...
                if (u_warp < window_size || u_warp >= width - window_size)
                    continue;
                updatePosteriorMinimum((__m128i *) (I2_line_addr + 16 * u_warp), d_curr, xmm1, xmm2, val, min_val,
                                       min_d);
            }
        }
...
void
Elas::computeDisparity(vector<support_pt> p_support, vector<triangle> tri, int32_t *disparity_grid, int32_t *grid_dims,
    ...
    for (uint32_t i = 0; i < tri.size(); i++) {
        ...
        float tri_v[3] = {static_cast<float>((float) p_support[c1].v), (float) p_support[c2].v,
