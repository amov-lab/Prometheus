...
int main(int argc, char **argv)
{
    ...
    nh.param<string>("rigid_body_name", rigid_body_name, "/ugv0");

    // 【订阅】optitrack估计位置
    ros::Publisher optitrack_pub = nh.advertise<geometry_msgs::PoseStamped>("/vrpn_client_node"+ rigid_body_name + "/pose", 1000);

    // 频率
    ros::Rate rate(100.0);

    Eigen::Vector3d pos_vicon_enu(1,2,0);

    Eigen::Vector3d Euler_vicon(0,0.0/180*M_PI,10.0/180*M_PI);

    Eigen::Quaterniond q_vicon = quaternion_from_rpy(Euler_vicon);  //欧拉角转四元数
