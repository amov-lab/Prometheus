...
void visualFeedbackCallback(const spirecv_msgs::TargetsInFrame::ConstPtr &msg)
{
    ...
    g_Detection_raw.category = "aruco";
    for(auto &target : msg->targets)//以嵌套方式获取msg中数据
    {
        if(target.tracked_id != goto_id && target.mode == false)//且判断，获取所选中二维码数据
            continue;
        g_Detection_raw = target;

    }
    // 相机坐标到机体坐标系x、y轴需要交换，并且方向相反。
    pos_body_enu_frame[0] = -g_Detection_raw.py;
    pos_body_enu_frame[1] = -g_Detection_raw.px;
    // <pose>0 0 0.05 0 1.5707963 0</pose>
    // 根据无人机pdf文件，下视觉相机在距离无人机质心0.05米位置
    // 无人机质心到地面距离是 0.09
    pos_body_enu_frame[2] = -(g_Detection_raw.pz + 0.05 - 0.09);
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        case TO_DESTINATION:
            ...
            uav_command.velocity_ref[0] = 0.5 * pos_body_enu_frame[0];
            uav_command.velocity_ref[1] = 0.5 * pos_body_enu_frame[1];
            uav_command.velocity_ref[2] = 0;
            uav_command.position_ref[2] = target_pos[2];
            info << "Find object,Go to the target point > velocity_x: " << uav_command.velocity_ref[0] << " [m/s] "
                 ...
                 << std::endl;
            PCOUT(1, GREEN, info.str());
            if (std::abs(uav_command.velocity_ref[0]) + std::abs(uav_command.velocity_ref[1]) < 0.06)
                run_status = LAND;
            break;
        case LAND:
            //坐标系
            ...
            uav_command.velocity_ref[0] = 0.5 * pos_body_enu_frame[0];
            uav_command.velocity_ref[1] = 0.5 * pos_body_enu_frame[1];
            uav_command.velocity_ref[2] = 0.2 * pos_body_enu_frame[2];
