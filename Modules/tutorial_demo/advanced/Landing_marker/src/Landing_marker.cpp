...
void landpadDetCb(const prometheus_msgs::TargetsInFrame::ConstPtr &msg)
{
    ...
    pos_body_enu_frame = g_R_Body_to_ENU * pos_body_frame;

    if (g_use_pad_height)
    {
        //若已知降落板高度，则无需使用深度信息。
        pos_body_enu_frame[2] = g_pad_height - g_UAVState.position[2];
    }

    // 机体惯性系 -> 惯性系
    pos_enu_frame[0] = g_UAVState.position[0] + pos_body_enu_frame[0];
    pos_enu_frame[1] = g_UAVState.position[1] + pos_body_enu_frame[1];
    pos_enu_frame[2] = g_UAVState.position[2] + pos_body_enu_frame[2];
    // 此降落方案不考虑偏航角
    att_enu_frame[2] = 0.0;

    if (g_Detection_raw.score >= 0.8 )
    {
...
inline void topicSub(ros::NodeHandle &nh)
{
    ...
    static ros::Subscriber landpad_det_sub = nh.subscribe<prometheus_msgs::TargetsInFrame>("/uav" + std::to_string(g_uav_id) + "/spirecv/landing_marker_detection", 10, landpadDetCb);
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        case TRACKING:
        {
            ...
            // 抵达上锁点,进入LANDING
            g_distance_to_pad = pos_body_enu_frame.norm();
            //　达到降落距离，上锁降落
            ...
            //　达到最低高度，上锁降落
            else if (abs(pos_body_enu_frame[2]) < g_arm_height_to_ground)
            {
                exec_state = LANDING;
            ...
            for (int i = 0; i < 3; i++)
            {
                g_command_now.velocity_ref[i] = g_kp_land[i] * pos_body_enu_frame[i];
            }
