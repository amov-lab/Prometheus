...
//---------------------------------------Output---------------------------------------------
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>回调函数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void TrackQRCb(const prometheus_msgs::TargetsInFrame::ConstPtr &msg)
{
    g_Detection_raw.category = "TrackQR";
...
inline void readParams(const ros::NodeHandle &nh)
{
    ...
    //跟踪间距
    nh.param<float>("Detection_distance", Detection_distance, 1.5);
...
inline void topicSub(ros::NodeHandle &nh)
{
    ...
    static ros::Subscriber TrackQR_sub = nh.subscribe<prometheus_msgs::TargetsInFrame>("/uav" + std::to_string(g_uav_id) + "/spirecv/aruco_detection_with_single_object_tracking", 10, TrackQRCb);
...
{
    ros::init(argc, argv, "Tracking_QR");
    ros::NodeHandle nh("~");
    ...
    while (ros::ok())
    {
        ...
        case TRACKING:
        {
            ...
            g_command_now.velocity_ref[0] = g_kp_detect[0] * (pos_body_frame[0] - Detection_distance);//g_kp_detect为速度跟踪比例控制参数
        ...
        case LOST:
        {
            ...
            if (lost_time < 30.0)
            {
                ...
                PCOUT(0, YELLOW, "Try reclicking the aruco");
            }
            else
            {
                ...
                PCOUT(0, YELLOW, "Reselection aruco");
            ...
            {
                exec_state = TRACKING;
                PCOUT(0, GREEN, "Regain the aruco.");
            }
            break;
        }
