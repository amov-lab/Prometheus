...
float ground_distance;  //地面跟踪高度判断距离
...
//---------------------------------------Output---------------------------------------------
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>回调函数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void NanoTrackCb(const prometheus_msgs::TargetsInFrame::ConstPtr &msg)
{
    g_Detection_raw.category = "NanoTrack";
    ...
    pos_body_frame[0] = g_Detection_raw.pz + g_camera_offset[0];
   ...
   //根据检测话题获得置信度score分数来判断检测结果的可靠性
    ...
    if(g_Detection_raw.score != 0 || g_Detection_raw.mode != true)
    {
...
inline void readParams(const ros::NodeHandle &nh)
{
    ...
    //跟踪距离保持
    nh.param<float>("keep_x_distance", keep_distance[0], 2.0);
    nh.param<float>("keep_y_distance", keep_distance[1], 0.0);
    nh.param<float>("keep_z_distance", keep_distance[2], 0.8);

    //地面跟踪高度判断距离
    nh.param<float>("ground_distance", ground_distance, 2.5);

    //追踪控制参数
    nh.param<float>("kpx_detect", g_kp_detect[0], 0.5);
...
inline void topicSub(ros::NodeHandle &nh)
{
    ...
    static ros::Subscriber NanoTrack_sub = nh.subscribe<prometheus_msgs::TargetsInFrame>("/uav" + std::to_string(g_uav_id) + "/spirecv/single_object_tracking", 10, NanoTrackCb);
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        case TRACKING:
        {
            ...
            {
                exec_state = LOST;
                PCOUT(0, YELLOW, "Lost the object.");
                break;
            ...
            //由于物体有天上和地上两种，根据地面跟踪高度判断距离做区分跟踪

            if(g_UAVState.position[2] > ground_distance)
            {
                
                //跟踪天上物体
                g_command_now.Agent_CMD = prometheus_msgs::UAVCommand::Move;
                g_command_now.Move_mode = prometheus_msgs::UAVCommand::XYZ_VEL;//速度模式跟踪
                g_command_now.velocity_ref[0] = g_kp_detect[0] * (pos_body_frame[0] - keep_distance[0]);//g_kp_detect为速度跟踪比例控制参数，根据实际情况可在launch文件直接调节
            ...
            else
            {
                ...
                g_command_now.velocity_ref[0] = g_kp_detect[0] * (pos_body_frame[0] - keep_distance[0]);//g_kp_detect为速度跟踪比例控制参数，根据实际情况可在launch文件直接调节
                g_command_now.velocity_ref[1] = g_kp_detect[1] * pos_body_frame[1];
                g_command_now.velocity_ref[2] = g_kp_detect[2] * (pos_body_frame[2] + keep_distance[2]);
                g_command_now.yaw_ref = 0;
                PCOUT(-1, GREEN, "object detected!");
                break;
        ...
        case LOST:
        {
            ...
            if (lost_time < 10.0)
            {
                ...
                PCOUT(0, YELLOW, "Try reclicking the object");
            }
            else
            {
                ...
                PCOUT(0, YELLOW, "Reselection object");
            }
            ...
            {
                exec_state = TRACKING;
                PCOUT(0, GREEN, "Regain the object.");
            }
            break;
        }
