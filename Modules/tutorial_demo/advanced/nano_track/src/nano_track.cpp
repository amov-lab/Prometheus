...
//---------------------------------------Output---------------------------------------------
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>回调函数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void NanoTrackCb(const spirecv_msgs::TargetsInFrame::ConstPtr &msg)
{
...
inline void topicSub(ros::NodeHandle &nh)
{
    ...
    static ros::Subscriber NanoTrack_sub = nh.subscribe<spirecv_msgs::TargetsInFrame>("/uav" + std::to_string(g_uav_id) + "/spirecv/single_object_tracking", 10, NanoTrackCb);
