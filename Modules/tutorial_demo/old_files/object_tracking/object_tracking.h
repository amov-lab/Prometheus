...
#define NODE_NAME "object_tracking"          // 节点名字
...
Eigen::Vector3d tracking_delta;
Eigen::Vector3d pos_des_prev;
double distance_to_setpoint;
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>回调函数<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
void aruco_cb(const prometheus_msgs::ArucoInfo::ConstPtr &msg)
{
    ...
    // aruco在无人机机体系下的位置
    vision_info.aruco_body[0] = - vision_info.aruco_info.position[2] + param.camera_offset[0];
    vision_info.aruco_body[1] = - vision_info.aruco_info.position[0] + param.camera_offset[1];
    vision_info.aruco_body[2] = - vision_info.aruco_info.position[1] + param.camera_offset[2];
...
void printf_param()
{
    ...
    cout << "kpx_track : "<< param.kp_track[0] << endl;
    cout << "kpy_track : "<< param.kp_track[1] << endl;
    cout << "kpz_track : "<< param.kp_track[2] << endl;
