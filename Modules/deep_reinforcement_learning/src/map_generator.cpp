...
void GenerateMap3()
{
    ...
    {
        generate_cylinder((double)8, (double)t);
    }

    // 第二排
    for (int t = -9; t <= 7; t=t+2)
    {
        generate_cylinder((double)6, (double)t);
    }

    for (int t = -8; t <= 8; t=t+2)
    {
        generate_cylinder((double)4, (double)t);
    }

    for (int t = -9; t <= 7; t=t+2)
    {
        generate_cylinder((double)2, (double)t);
    }

    for (int t = -8; t <= 8; t=t+2)
    {
        generate_cylinder((double)0, (double)t);
    }
    ...
    cout << GREEN << "[map_generator] Finished generate map 3." << TAIL << endl;
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i <= swarm_num; i++)
    {
        uav_id[i] = i;
        // 【订阅】里程计数据
        uav_odom_sub[i] = nh.subscribe<nav_msgs::Odometry>("/uav" + std::to_string(i) + "/odom", 1, boost::bind(&uav_odom_cb, _1, uav_id[i]));
        // 【发布】模拟的局部点云信息
