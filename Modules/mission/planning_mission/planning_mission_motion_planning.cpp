...
void Fast_planner()
{
    ...
    Command_Now.Reference_State.position_ref[i]     = 1/120.0 * alpha[i] * pow(time_trajectory,5) + 1/24.0 * beta[i] * pow(time_trajectory,4) + 1/6.0 * gamma[i] * pow(time_trajectory,3) + v_0[i] * time_trajectory + p_0[i];
    Command_Now.Reference_State.velocity_ref[i]     =  1/24.0 * alpha[i] * pow(time_trajectory,4) +  1/6.0 * beta[i] * pow(time_trajectory,3) + 1/2.0 * gamma[i] * pow(time_trajectory,2) + v_0[i];
    Command_Now.Reference_State.acceleration_ref[i] =   1/6.0 * alpha[i] * pow(time_trajectory,3) +  1/2.0 * beta[i] * pow(time_trajectory,2) + gamma[i] * time_trajectory;
