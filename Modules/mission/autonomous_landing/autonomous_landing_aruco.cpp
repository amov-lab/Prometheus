...
void control_cb(const ros::TimerEvent& e)
{
    ...
    if(exec_state == EXEC_STATE::HORIZON_APPROACH)
    {
        ...
        Command_Now.Reference_State.position_ref[2] = 0.7;
        command_pub.publish(Command_Now);
    }
    //maxi add 第一次水平对齐，在0.7米高处，基于二维码ENU坐标，直接进行指点让无人机飞到二维码上方
    else if(exec_state == EXEC_STATE::HORIZON_APPROACH_POSCTL_FIRST)
    {
        Command_Now.header.stamp = ros::Time::now();
        Command_Now.Command_ID   = Command_Now.Command_ID + 1;
        Command_Now.source = NODE_NAME;
        ...
        Command_Now.Reference_State.Move_frame = prometheus_msgs::PositionReference::ENU_FRAME;
        Command_Now.Reference_State.Move_mode = prometheus_msgs::PositionReference::XYZ_POS;  
        Command_Now.Reference_State.position_ref[0] = marker_enu[0];
        Command_Now.Reference_State.position_ref[1] = marker_enu[1];
        Command_Now.Reference_State.position_ref[2] = 0.7;
 
        command_pub.publish(Command_Now);

    }
    //maxi add 第二次水平对齐，在0.35米高处，基于二维码ENU坐标，进行指点让无人机再次水平对齐二维码
    else if(exec_state == EXEC_STATE::HORIZON_APPROACH_POSCTL_SECOND)
    {
        Command_Now.header.stamp = ros::Time::now();
        Command_Now.Command_ID   = Command_Now.Command_ID + 1;
        Command_Now.source = NODE_NAME;
        ...
        Command_Now.Reference_State.position_ref[0] = marker_enu[0];
        Command_Now.Reference_State.position_ref[1] = marker_enu[1];
        Command_Now.Reference_State.position_ref[2] = 0.35;
 
        command_pub.publish(Command_Now);

    }
    //maxi add 在离地35厘米的时候再水平靠齐一次，此为比例控制输出期望速度进行水平靠齐，在离二维码35厘米高处用此方法进行水平靠齐易丢失二维码，建议采用指点方式进行水平对齐
    else if(exec_state == EXEC_STATE::HORIZON_APPROACH_LAST)
    {
        // 机体系速度控制
        Command_Now.header.stamp = ros::Time::now();
        Command_Now.Command_ID   = Command_Now.Command_ID + 1;
     ...
     switch (exec_state)
    {
        ...
        case  SEARCH:
           ...
           else  if(is_detected)
            {
                ...
                exec_state = EXEC_STATE::HORIZON_APPROACH_POSCTL_FIRST;
                break;
            }
 
            if(search_id == 0)
            {
                search_point << 0.0, 0.0, 0.7;
            }else if(search_id == 1)
            {
                search_point << 0.0, 0.6, 0.7;
            }else if (search_id == 2)
            {
                search_point << 0.6, 0.6, 0.7;
            }else if (search_id == 3)
            {
                search_point << 0.6, 0.0, 0.7;
            }else if (search_id == 4)
            {
                search_point << 1.2, 0.0, 0.7;
            }else if (search_id == 5)
            {
                search_point << 1.2, 0.6, 0.7;
            }
        ...
        case HORIZON_APPROACH:
            ...
            else if(abs(marker_body[0])<0.05 && abs(marker_body[1])<0.05)
            {
        ...
        //maxi add
        case HORIZON_APPROACH_POSCTL_FIRST:
             // 水平距离达到阈值，进入下降阶段，由于有时候y偏差可能相对略大，所以第一次水平对齐进入下降阶段的y的阈值由5厘米改为9厘米，为了能更快进入下降阶段。
             if(abs(marker_body[0])<0.05 && abs(marker_body[1])<0.09)
            {
                exec_state = EXEC_STATE::DECEND;
            }
        break;
        ...
        case DECEND:
            ...
            if(_DroneState.position[2]<0.37)
            {
                exec_state = EXEC_STATE::HORIZON_APPROACH_POSCTL_SECOND;
            }
 
        break;

        //maxi add
        case HORIZON_APPROACH_LAST:
        ...
        //maxi add
        case HORIZON_APPROACH_POSCTL_SECOND:
            // 水平距离达到阈值，切land
            if(abs(marker_body[0])<0.05 && abs(marker_body[1])<0.05)
            {
                exec_state = EXEC_STATE::DISARM;
            }
