...
--tab -e 'bash -c "sleep 0; roslaunch airsim_ros_pkgs airsim_node.launch output:=screen host:=$PX4_SIM_HOST_ADDR; exit; exec bash"' \
--tab -e 'bash -c "sleep 3; roslaunch prometheus_airsim airsim_scan_to_octomap_3uav.launch; exit; exec bash"' \
--tab -e 'bash -c "sleep 3; roslaunch prometheus_airsim airsim_sitl_ego_lidar_octomap_3uav.launch; exit; exec bash"' \
