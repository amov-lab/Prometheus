...
--tab -e 'bash -c "sleep 1.5s; source ~/spirecv-ros/devel/setup.bash; roslaunch prometheus_demo aruco_tracking_prosim.launch; exit; exec bash"' \
--tab -e 'bash -c "sleep 1.5s; export LD_LIBRARY_PATH=/usr/local/cuda/lib64:/home/amov/TensorRT-8.6.1.6/lib:/opt/intel/openvino_2022/runtime/lib/intel64:$LD_LIBRARY_PATH; source ~/spirecv-ros/devel/setup.bash; roslaunch spirecv_ros prosim_aruco_detection_with_d435i.launch; exit; exec bash"' \
