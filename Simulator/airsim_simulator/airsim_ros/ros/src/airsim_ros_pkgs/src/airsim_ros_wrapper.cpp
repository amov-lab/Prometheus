        ...
        for (const auto& [sensor_name, sensor_setting] : vehicle_setting->sensors) {
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Lidar: {
                    ...
                    {
                        sensor_publisher.publisher = nh_private_.advertise<sensor_msgs::LaserScan>(curr_vehicle_name + "/lidarLaserScan/" + sensor_name, 10);
                    }
                    else
                    {
                        sensor_publisher.publisher = nh_private_.advertise<sensor_msgs::PointCloud2>(curr_vehicle_name + "/lidarPointCloud2/" + sensor_name, 10);
                    }
                    break;
                }
                default: {
...
tf2::Quaternion AirsimROSWrapper::fromYaw(double yaw, const tf2::Quaternion &q) const
{
    double halfYaw = yaw / 2.0;
    return tf2::Quaternion(q.x(), q.y(), std::sin(halfYaw), std::cos(halfYaw));
}

tf2::Quaternion AirsimROSWrapper::calculateQ2(const tf2::Quaternion &Q1) const
{
    tf2::Quaternion temp_q = Q1;
    tf2::Matrix3x3 m(temp_q); 
    double roll, pitch, yaw;
    m.getRPY(roll, pitch, yaw); 

    double yawQ2 = yaw - M_PI/2; 

    tf2::Quaternion q_new;
    q_new.setRPY(pitch, roll, yawQ2);
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_multirotor_state(const msr::airlib::MultirotorState& drone_state) const
{
    ...
    if (isENU_) {
        ...
        tf2::Quaternion Q1 = tf2::Quaternion(odom_msg.pose.pose.orientation.y, 
                        odom_msg.pose.pose.orientation.x, 
                        -odom_msg.pose.pose.orientation.z, 
                        odom_msg.pose.pose.orientation.w);

        tf2::Quaternion Q2 = calculateQ2(Q1);
...
sensor_msgs::PointCloud2 AirsimROSWrapper::get_lidarPointCloud2_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name, const std::string& sensor_name) const
{
...
airsim_ros_pkgs::LivoxCustomMsg AirsimROSWrapper::get_lidar_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name, const std::string& sensor_name) const
{
    ...
    lidar_msg_livox.timebase = tn.tv_nsec;
    lidar_msg_livox.header.stamp = ros::Time::now();

    sensor_msgs::PointCloud2 lidar_msg;
    lidar_msg.header.stamp = ros::Time::now();
    lidar_msg.header.frame_id = vehicle_name + "/" + sensor_name;
    if (lidar_data.point_cloud.size() > 3) {
        // std::cout << "PointNum: " << lidar_data.point_cloud.size() << " && " << "LineNum: " << 0 << std::endl;        
    ...
    //             auto transformStampedENU = tf_buffer_.lookupTransform(AIRSIM_FRAME_ID, vehicle_name + "/odom_local_enu", ros::Time(0), ros::Duration(1));

    //             geometry_msgs::TransformStamped modifiedTransform = transformStampedENU;
    //             modifiedTransform.transform.translation.x = 0.0;
    //             modifiedTransform.transform.translation.y = 0.0;
    //             modifiedTransform.transform.translation.z = 0.0;
    //             tf2::doTransform(lidar_msg, lidar_msg_enu, modifiedTransform);
    ...
    //     }
        ...
        const float* ptr = reinterpret_cast<const float*>(&lidar_msg.data[0]);
        
        // 计算点的数量
        size_t point_step = lidar_msg.point_step / sizeof(float); // 每个点的步长
        size_t num_points = lidar_msg.height * lidar_msg.width;
        // std::cout << "PointNum " << num_points << std::endl;
        // 遍历每个点
        if(num_points == lidar_data.lines.size())
        {
            for (size_t i = 0; i < num_points; ++i) {
                // 假设每个点包含 x, y, z 坐标
                // float x = ptr[i * point_step];     // x坐标
                ...
                lidar_msg_livox.points.push_back(pt);
                
                //std::cout << "Point " << i << ": (" << x << ", " << y << ", " << z << ")" << std::endl;
            }

            lidar_msg_livox.point_num = lidar_msg_livox.points.size();
        }  
    }
    else {
        // msg = []
    ...
    return lidar_msg_livox;
}

sensor_msgs::LaserScan AirsimROSWrapper::get_lidarLaserScan_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& vehicle_name, const std::string& sensor_name, const msr::airlib::Settings& Settings) const
{
    sensor_msgs::LaserScan lidar_msg_LaserScan;
    lidar_msg_LaserScan.header.frame_id = vehicle_name + "/" + sensor_name;
    lidar_msg_LaserScan.header.stamp = ros::Time::now();
    lidar_msg_LaserScan.angle_min = 0;
    // lidar_msg_LaserScan.angle_max = 2 * M_PI;
    float HorizontalFOVEnd = Settings.getInt("HorizontalFOVEnd", HorizontalFOVEnd);
    lidar_msg_LaserScan.angle_max = M_PI / 180 * HorizontalFOVEnd;
    sensor_msgs::PointCloud2 lidar_msg;
    lidar_msg.header.stamp = ros::Time::now();
    lidar_msg.header.frame_id = vehicle_name + "/" + sensor_name;
    if (lidar_data.point_cloud.size() > 3) {
        ...
        if(num_points == lidar_data.lines.size())
        {
            // lidar_msg_LaserScan.angle_increment = (lidar_msg_LaserScan.angle_max - lidar_msg_LaserScan.angle_min) / num_points;
            lidar_msg_LaserScan.angle_increment = (lidar_msg_LaserScan.angle_min - lidar_msg_LaserScan.angle_max ) / num_points;
            // ROS_INFO("num_points: %.6f ", num_points);
            lidar_msg_LaserScan.range_min = 0.0;
            float MaxRange = Settings.getFloat("Range", MaxRange);
            lidar_msg_LaserScan.range_max = MaxRange;
            lidar_msg_LaserScan.ranges.resize(num_points);
            for (size_t i = 0; i < num_points; ++i) {
                // 假设每个点包含 x, y, z 坐标
                // float x = ptr[i * point_step];     // x坐标
                // float y = ptr[i * point_step + 1]; // y坐标
                // float z = ptr[i * point_step + 2]; // z坐标
                auto uu = std::sqrt(ptr[i * point_step] * ptr[i * point_step] + ptr[i * point_step + 1] * ptr[i * point_step + 1] + ptr[i * point_step + 2] * ptr[i * point_step + 2]);
                // std::cout << "Distance: " << uu << std::endl;
                lidar_msg_LaserScan.ranges[i] = std::sqrt(ptr[i * point_step] * ptr[i * point_step] + ptr[i * point_step + 1] * ptr[i * point_step + 1] + ptr[i * point_step + 2] * ptr[i * point_step + 2]);
                // if(i < 2)
                // {
                //     std::cout << "Point " << i << ": (" << x << ", " << y << ", " << z << ")" << std::endl;
                // }
...
void AirsimROSWrapper::img_response_timer_cb(const ros::TimerEvent& event)
{
    ...
    // ROS_INFO("Callback execution time: %.6f seconds", duration.toSec());
}
        ...
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            if (!vehicle_name_ptr_pair.second->lidar_pubs.empty()) {
                for (auto& lidar_publisher : vehicle_name_ptr_pair.second->lidar_pubs) {
                    ...
                    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
                        if (vehicle_name_ptr_pair.first == curr_vehicle_elem.first){
                            auto& vehicle_setting = curr_vehicle_elem.second;
                            for (const auto& [sensor_name, sensor_setting] : vehicle_setting->sensors) {
                                if (sensor_name == lidar_publisher.sensor_name && sensor_setting->enabled)
                                {
                                    auto lidar_data = airsim_client_lidar_.getLidarData(lidar_publisher.sensor_name, vehicle_name_ptr_pair.first);
                                    
                                    // ROS_INFO("Callback execution time: %.6f == %.6f == %.6f == %.6f", lidar_data.pose.orientation.x(), lidar_data.pose.orientation.y(), lidar_data.pose.orientation.z(), lidar_data.pose.orientation.w());
                                    auto lidar_setting = *static_cast<LidarSetting*>(sensor_setting.get());
                                    float SensorDataType = lidar_setting.settings.getInt("SensorDataType", SensorDataType);
                                    if (SensorDataType == 2)
                                    {   
                                        sensor_msgs::LaserScan lidarPoint_msg = get_lidarLaserScan_msg_from_airsim(lidar_data, vehicle_name_ptr_pair.first, lidar_publisher.sensor_name, lidar_setting.settings);
                                        lidar_publisher.publisher.publish(lidarPoint_msg);
                                    }
                                    ...
                                    {   
                                        sensor_msgs::PointCloud2 lidarPoint_msg = get_lidarPointCloud2_msg_from_airsim(lidar_data, vehicle_name_ptr_pair.first, lidar_publisher.sensor_name);
                                        lidar_publisher.publisher.publish(lidarPoint_msg);
                                    }
                                }
                            }
                                                                        ...
                                                                        const std::string frame_id)
{
    ...
    cv::Mat depth_img_16UC1;
    depth_img.convertTo(depth_img_16UC1, CV_16UC1, 1000.0); // Adjust scaling factor as needed
    sensor_msgs::ImagePtr depth_img_msg = cv_bridge::CvImage(std_msgs::Header(), "16UC1", depth_img_16UC1).toImageMsg();
