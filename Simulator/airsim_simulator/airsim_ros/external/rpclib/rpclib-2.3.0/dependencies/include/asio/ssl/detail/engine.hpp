...
public:
  ...
  ASIO_DECL clmdep_asio::error_code set_verify_callback(
  ...
  ASIO_DECL want shutdown(clmdep_asio::error_code& ec);

  // Write bytes to the SSL session.
  ASIO_DECL want write(const clmdep_asio::const_buffer& data,
      clmdep_asio::error_code& ec, std::size_t& bytes_transferred);

  // Read bytes from the SSL session.
  ASIO_DECL want read(const clmdep_asio::mutable_buffer& data,
      clmdep_asio::error_code& ec, std::size_t& bytes_transferred);
  ...
  ASIO_DECL clmdep_asio::const_buffer put_input(
      const clmdep_asio::const_buffer& data);

  // Map an error::eof code returned by the underlying transport according to
  // the type and state of the SSL session. Returns a const reference to the
  // error code object, suitable for passing to a completion handler.
  ASIO_DECL const clmdep_asio::error_code& map_error_code(
