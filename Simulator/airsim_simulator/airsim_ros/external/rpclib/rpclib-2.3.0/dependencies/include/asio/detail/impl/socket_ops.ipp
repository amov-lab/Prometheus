...
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  ec = clmdep_asio::error_code(WSAGetLastError(),
...
socket_type accept(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
...
socket_type sync_accept(socket_type s, state_type state,
    socket_addr_type* addr, std::size_t* addrlen, clmdep_asio::error_code& ec)
{
  // Accept a socket.
  for (;;)
  {
...
void complete_iocp_accept(socket_type s,
    ...
    socket_type new_socket, clmdep_asio::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = clmdep_asio::error::connection_aborted;
...
bool non_blocking_accept(socket_type s,
    ...
    clmdep_asio::error_code& ec, socket_type& new_socket)
{
  for (;;)
  {
...
int close(socket_type s, state_type& state,
    bool destruction, clmdep_asio::error_code& ec)
{
  int result = 0;
  if (s != invalid_socket)
  {
...
int shutdown(socket_type s, int what, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
    ...
    std::size_t addrlen, clmdep_asio::error_code& ec)
{
  ...
  else if (ec == clmdep_asio::error::try_again)
    ec = clmdep_asio::error::no_buffer_space;
    ...
    std::size_t addrlen, clmdep_asio::error_code& ec)
{
  ...
  if (ec != clmdep_asio::error::in_progress
      && ec != clmdep_asio::error::would_block)
  {
  ...
  ec = clmdep_asio::error_code(connect_error,
      clmdep_asio::error::get_system_category());
}

#if defined(ASIO_HAS_IOCP)

void complete_iocp_connect(socket_type s, clmdep_asio::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  switch (ec.value())
  {
  ...
  case ERROR_NETWORK_UNREACHABLE:
    ec = clmdep_asio::error::network_unreachable;
    break;
  case ERROR_HOST_UNREACHABLE:
    ec = clmdep_asio::error::host_unreachable;
    break;
  case ERROR_SEM_TIMEOUT:
    ec = clmdep_asio::error::timed_out;
    break;
  default:
    break;
  }
...
bool non_blocking_connect(socket_type s, clmdep_asio::error_code& ec)
{
...
int socketpair(int af, int type, int protocol,
    socket_type sv[2], clmdep_asio::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(af);
  (void)(type);
...
bool sockatmark(socket_type s, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
...
# if defined(ENOTTY)
  if (ec.value() == ENOTTY)
    ec = clmdep_asio::error::not_socket;
...
size_t available(socket_type s, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
...
int listen(socket_type s, int backlog, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
...
signed_size_type recv(socket_type s, buf* bufs, size_t count,
    int flags, clmdep_asio::error_code& ec)
{
  clear_last_error();
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
...
size_t sync_recv(socket_type s, state_type state, buf* bufs,
    size_t count, int flags, bool all_empty, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
...
void complete_iocp_recv(state_type state,
    ...
    clmdep_asio::error_code& ec, size_t bytes_transferred)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
  {
...
signed_size_type recvmsg(socket_type s, buf* bufs, size_t count,
    int in_flags, int& out_flags, clmdep_asio::error_code& ec)
{
  clear_last_error();
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  out_flags = 0;
...
int setsockopt(socket_type s, state_type& state, int level, int optname,
    const void* optval, std::size_t optlen, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
...
int getsockopt(socket_type s, state_type state, int level, int optname,
    void* optval, size_t* optlen, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
...
int getpeername(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, bool cached, clmdep_asio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = clmdep_asio::error::bad_descriptor;
  ...
  if (cached)
  {
    ...
    {
      ec = clmdep_asio::error::not_connected;
      return socket_error_retval;
    }
...
const char* inet_ntop(int af, const void* src, char* dest, size_t length,
    unsigned long scope_id, clmdep_asio::error_code& ec)
{
  clear_last_error();
#if defined(ASIO_WINDOWS_RUNTIME)
...
int gethostname(char* name, int namelen, clmdep_asio::error_code& ec)
{
  clear_last_error();
#if defined(ASIO_WINDOWS_RUNTIME)
  try
...
inline clmdep_asio::error_code translate_netdb_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case HOST_NOT_FOUND:
    return clmdep_asio::error::host_not_found;
  case TRY_AGAIN:
    return clmdep_asio::error::host_not_found_try_again;
  case NO_RECOVERY:
    return clmdep_asio::error::no_recovery;
  case NO_DATA:
    return clmdep_asio::error::no_data;
  default:
    ASIO_ASSERT(false);
    return clmdep_asio::error::invalid_argument;
  }
}

inline hostent* gethostbyaddr(const char* addr, int length, int af,
    hostent* result, char* buffer, int buflength, clmdep_asio::error_code& ec)
{
  clear_last_error();
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
...
inline hostent* gethostbyname(const char* name, int af, struct hostent* result,
    char* buffer, int buflength, int ai_flags, clmdep_asio::error_code& ec)
{
  clear_last_error();
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    if (hptr == 0)
    {
      ...
      freeaddrinfo_emulation(aihead);
      gai_free(canon);
      if (ec == clmdep_asio::error::host_not_found)
        return EAI_NONAME;
      if (ec == clmdep_asio::error::host_not_found_try_again)
        return EAI_AGAIN;
      if (ec == clmdep_asio::error::no_recovery)
        return EAI_FAIL;
      if (ec == clmdep_asio::error::no_data)
        return EAI_NONAME;
      return EAI_NONAME;
    }
...
inline clmdep_asio::error_code getnameinfo_emulation(
    ...
    clmdep_asio::error_code& ec)
{
  ...
  case ASIO_OS_DEF(AF_INET):
    ...
    {
      return ec = clmdep_asio::error::invalid_argument;
    }
  ...
  case ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    return ec = clmdep_asio::error::address_family_not_supported;
  }
  ...
  if (host && hostlen > 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          return ec = clmdep_asio::error::host_not_found;
        }
  ...
  if (serv && servlen > 0)
  {
    if (flags & NI_NUMERICSERV)
    {
      if (servlen < 6)
      {
        return ec = clmdep_asio::error::no_buffer_space;
      }
#if defined(ASIO_HAS_SECURE_RTL)
...
inline clmdep_asio::error_code translate_addrinfo_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case EAI_FAIL:
    return clmdep_asio::error::no_recovery;
  case EAI_FAMILY:
    return clmdep_asio::error::address_family_not_supported;
  case EAI_MEMORY:
    return clmdep_asio::error::no_memory;
  case EAI_NONAME:
  ...
  case EAI_NODATA:
    ...
    return clmdep_asio::error::host_not_found;
  case EAI_SERVICE:
    return clmdep_asio::error::service_not_found;
  case EAI_SOCKTYPE:
    return clmdep_asio::error::socket_type_not_supported;
  default: // Possibly the non-portable EAI_SYSTEM.
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
    return clmdep_asio::error_code(
        WSAGetLastError(), clmdep_asio::error::get_system_category());
#else
    return clmdep_asio::error_code(
...
clmdep_asio::error_code getaddrinfo(const char* host,
    const char* service, const addrinfo_type& hints,
    addrinfo_type** result, clmdep_asio::error_code& ec)
{
...
clmdep_asio::error_code background_getaddrinfo(
...
clmdep_asio::error_code getnameinfo(const socket_addr_type* addr,
    std::size_t addrlen, char* host, std::size_t hostlen,
    char* serv, std::size_t servlen, int flags, clmdep_asio::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
# if defined(ASIO_HAS_GETADDRINFO)
  // Building for Windows XP, Windows Server 2003, or later.
...
clmdep_asio::error_code sync_getnameinfo(
    const socket_addr_type* addr, std::size_t addrlen,
    char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, clmdep_asio::error_code& ec)
{
...
clmdep_asio::error_code background_getnameinfo(
    const weak_cancel_token_type& cancel_token,
    const socket_addr_type* addr, std::size_t addrlen,
    char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, clmdep_asio::error_code& ec)
