    ...
    public:
        ...
        */
        ...
        virtual const LidarData& getLidarData(const std::string& lidar_name) const
        {
            ...
            //throw VehicleControllerException(Utils::stringf("No lidar with name %llu == %llu == %llu", lidar->getOutput().lines.size(), lidar->getOutput().point_cloud.size(), lidar->getOutput().segmentation.size()));
            // std::string msg1 = std::to_string(lidar->getOutput().point_cloud.size());
            // std::string msg2 = std::to_string(lidar->getOutput().segmentation.size());
            // std::string msg3 = std::to_string(lidar->getOutput().lines.size());
