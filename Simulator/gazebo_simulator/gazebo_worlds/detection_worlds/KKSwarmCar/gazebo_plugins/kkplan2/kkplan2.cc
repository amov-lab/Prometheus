...
namespace gazebo
{
  class kkplan2 : public ModelPlugin  //修改此处的类名为  move  ，要和最后注册的名称一样
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
    {
        ...
        gazebo::common::PoseAnimationPtr anim(new gazebo::common::PoseAnimation("test", 4000.0, true));  
  ...
  double l[40] = {3.5,3.6,3.7,3.8,3.9,4,4.1,4.2,4.3,4.4,4.5,4.4,4.3,4.2,4.1,4,3.9,3.8,3.7,3.6,3.5,3.6,3.7,3.8,3.9,4,4.1,4.2,4.3,4.4,4.5,4.4,4.3,4.2,4.1,4,3.9,3.8,3.7,3.6};
        //设置模型到达某位姿的时刻   可以任意修改
	for(tme=0;tme<=1200;tme++){
		key = anim->CreateKeyFrame(tme);
        	key->Translation(ignition::math::Vector3d(j[tme%40],k[tme%40], l[tme%40]));   //xyz
  ...
  GZ_REGISTER_MODEL_PLUGIN(kkplan2)
}
